...
void Futball::read_one_week(string file_name)
{
    ...
    {
        game_data *game = read_from_file(line_of_file);
...
game_data *Futball::read_from_file(string line_of_file)
{
    vector<string> result, players_team_1, players_team_2;
    line_of_file += " ";
    vector<string> teams_name = get_names(line_of_file, ':');
    game_data *game = make_game(teams_name);
    game->set_result(result = get_names(line_of_file, ':'));
    ...
    game->set_players_goals_assists(get_names(line_of_file, ';'));
    players_team_1 = get_names(line_of_file, ';');
    players_team_2 = get_names(line_of_file, ';');
    game->give_players_point(players_team_1, players_team_2, result);
    return game;
}
...
void Futball::read_premier_leage_data()
{
    ...
    while (getline(input, line_of_file))
    {
        ...
        vector<string> players_name_cost;
        Real_Team *temp_team = new Real_Team;
        players_name_cost = get_names(line_of_file, ';');
        temp_team->set_goalkeepers(team_name[0], players_name_cost);
        players_name_cost = get_names(line_of_file, ';');
        temp_team->set_defenders(team_name[0], players_name_cost);
        players_name_cost = get_names(line_of_file, ';');
        temp_team->set_midfielders(team_name[0], players_name_cost);
        players_name_cost = get_names(line_of_file, ';');
        temp_team->set_forwards(team_name[0], players_name_cost);
...
void Futball::post_orders(string order)
{
    ...
    if (order == "set_captain")
        set_captain();
}

void Futball::set_captain()
{
    string order, name;
    cin >> order >> order;
    getline(cin, name);
    ...
    if (!which_team_is_login()->have_player(name))
    {
        cout << "Not Found\n";
        return;
    }
    which_team_is_login()->set_team_captain(find_player_by_name(name));
    cout << "OK\n";
}
...
void Futball::get_orders(string order)
{
    ...
    if (order == "squad")
        make_user_team();
    if (order == "show_budget")
        show_budget();
}

void Futball::show_budget()
{
    if (!is_user_online)
    {
        cout << "Permission Denied\n";
    ...
    int budget = which_team_is_login()->get_budget();
    cout << budget << "\n";
}
...
void Futball::make_user_team()
{
    ...
    if (players_of_fantasy_team.size() < MAX_PLAYERS_OF_FANTASY_TEAM || weeks.size() == NO_WEEK_PASSED)
    {
        cout << "Empty\n";
        return;
    }
...
void Futball::print_user_team(vector<Player *> players, User_Team *user_team)
{
    ...
    {
        if (user_team->get_captain() == players[i])
            cout << position_names[i] << players[i]->get_name() << " (CAPTAIN)\n";
        else
            cout << position_names[i] << players[i]->get_name() << "\n";
    }
    cout << "Total Points: " << floorf(user_team->get_team_point() * 10) / 10 << "\n";
    cout << "Team Cost: " << user_team->calculate_team_cost() << "\n";
}
...
vector<User_Team *> Futball::sort_user_teams_by_their_points(vector<User_Team *> teams)
{
    for (int i = 0; i < teams.size(); i++)
    {
        ...
        {
            if (floorf(teams[i]->get_team_point() * 10) / 10 < floorf(teams[j]->get_team_point() * 10) / 10)
            {
                swap(teams[i], teams[j]);
            }
            else if (floorf(teams[i]->get_team_point() * 10) / 10 == floorf(teams[j]->get_team_point() * 10) / 10)
...
{
    int week_number = 0;
    if (order_words.size() == 3 && order_words[1] == "week_num")
...
void Futball::print_players_of_team(vector<Player *> players_in_position, vector<string> order_words)
{
    ...
    {
        sorted_players[i]->print_specifications(i + 1);
    }
}

vector<Player *> Futball::sort_players_by_their_rank(vector<Player *> _players)
{
    vector<Player *> players = _players;
    for (int i = 0; i < players.size(); i++)
    {
        for (int j = i + 1; j < players.size(); j++)
        {
            if (floorf(players[i]->get_average_score() * 10) / 10 < floorf(players[j]->get_average_score() * 10) / 10)
                swap(players[i], players[j]);
            else if (floorf(players[i]->get_average_score() * 10) / 10 == floorf(players[j]->get_average_score() * 10 ) / 10)
                if (players[i]->get_name() > players[j]->get_name())
                    swap(players[i], players[j]);
        }
    }
...
void Futball::make_position_of_the_week(int position, int number_of_players, int week_number, string position_name)
{
    ...
    players_in_position = sort_players_by_their_rank_and_name(players_in_position, week_number);
    print_team_of_the_week(players_in_position, position_name, week_number, number_of_players);
}
...
vector<Player *> Futball::sort_players_by_their_rank_and_name(vector<Player *> players, int week_number)
{
    for (int i = 0; i < players.size(); i++)
    {
        for (int j = i + 1; j < players.size(); j++)
        {
            if (floorf(players[i]->get_final_score()[week_number] * 10 ) / 10 < floorf(players[j]->get_final_score()[week_number] * 10) / 10)
                swap(players[i], players[j]);
            else if (floorf(players[i]->get_final_score()[week_number] * 10 ) / 10 == floorf(players[j]->get_final_score()[week_number] * 10) / 10)
                if (players[i]->get_name() > players[j]->get_name())
                    swap(players[i], players[j]);
        }
    }
...
void Futball::print_team_of_the_week(vector<Player *> max_score_player, string position_name, int week_number, int number_of_players)
{
    for (int i = 0; i < number_of_players; i++)
    {
        if (number_of_players > 1)
            cout << position_name << i + 1 << ": " << max_score_player[i]->get_name() << " | Score: " << floorf(max_score_player[i]->get_final_score()[week_number] * 10) / 10 << "\n";
        else
            cout << position_name << ": " << max_score_player[i]->get_name() << " | Score: " << floorf(max_score_player[i]->get_final_score()[week_number] * 10) / 10 << "\n";
    }
}
